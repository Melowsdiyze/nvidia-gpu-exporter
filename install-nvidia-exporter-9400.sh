#!/bin/bash
# =============================================================================
# NVIDIA Custom Python Exporter — Install Script
# Port: 9400 (compatible with DCGM Exporter port)
# Replaces: nvcr.io/nvidia/k8s/dcgm-exporter Docker container
# Author: Custom GPU Monitoring Solution
# =============================================================================
...
# ─── Configuration ────────────────────────────────────────────────────────────
INSTALL_DIR="${INSTALL_DIR:-/opt/nvidia-exporter}"
LOG_DIR="${LOG_DIR:-/var/log/nvidia-exporter}"
EXPORTER_PORT="${NVIDIA_EXPORTER_PORT:-9400}"
EXPORTER_USER="${EXPORTER_USER:-nvidia-exporter}"
SERVICE_NAME="nvidia-exporter"
PYTHON_MIN_VERSION="3.8"
REPO_SCRIPT_URL=""   # Set this if pulling from a repo; leave empty to use local file
LOCAL_SCRIPT="${LOCAL_SCRIPT:-}"  # Path to nvidia_exporter.py if available locally
...
header "NVIDIA Custom Python Exporter Installer"
echo -e "  Install dir : ${INSTALL_DIR}"
echo -e "  Log dir     : ${LOG_DIR}"
echo -e "  Port        : ${EXPORTER_PORT}"
echo -e "  Service     : ${SERVICE_NAME}"
echo ""

# ─── 1. Check nvidia-smi ──────────────────────────────────────────────────────
header "Step 1: Checking NVIDIA Driver"
if ! command -v nvidia-smi &>/dev/null; then
    warn "nvidia-smi not found in PATH. The exporter will still install but will report GPU unavailable."
    warn "Please install the NVIDIA driver first: https://www.nvidia.com/Download/index.aspx"
else
    DRIVER_VER=$(nvidia-smi --query-gpu=driver_version --format=csv,noheader 2>/dev/null | head -1 || echo "unknown")
    GPU_COUNT=$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | wc -l || echo 0)
    success "nvidia-smi found — Driver: ${DRIVER_VER}, GPUs: ${GPU_COUNT}"
fi

# ─── 2. Stop existing service / DCGM container ────────────────────────────────
header "Step 2: Stopping Existing GPU Exporters on Port ${EXPORTER_PORT}"

# Track whether nvidia-monitoring container was running so we can restart it after install
NVIDIA_MONITORING_WAS_RUNNING=false

# Stop nvidia-monitoring container if running (will be restarted after install)
if command -v docker &>/dev/null; then
    if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx "nvidia-monitoring"; then
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "nvidia-monitoring"; then
            info "Container 'nvidia-monitoring' is running — stopping before install..."
            docker stop nvidia-monitoring 2>/dev/null || true
            NVIDIA_MONITORING_WAS_RUNNING=true
            success "Container 'nvidia-monitoring' stopped (will be restarted after install)"
        else
            info "Container 'nvidia-monitoring' exists but is not running — skipping stop"
        fi
    else
        info "Container 'nvidia-monitoring' not found — proceeding directly"
    fi

    # Stop other known DCGM containers (these are removed, not restarted)
    for cname in dcgm-exporter nvidia-dcgm-exporter; do
        if docker ps -q -f name="^/${cname}$" 2>/dev/null | grep -q .; then
            info "Stopping Docker container: ${cname}"
            docker stop "${cname}" 2>/dev/null && docker rm "${cname}" 2>/dev/null || true
            success "Stopped and removed container: ${cname}"
        fi
    done
fi
...
if systemctl is-active --quiet "${SERVICE_NAME}" 2>/dev/null; then
    info "Stopping existing systemd service: ${SERVICE_NAME}"
...
if command -v fuser &>/dev/null; then
    fuser -k "${EXPORTER_PORT}/tcp" 2>/dev/null || true
fi

# ─── 3. Python check ──────────────────────────────────────────────────────────
header "Step 3: Checking Python"
...
for py in python3.11 python3.10 python3.9 python3.8 python3; do
    if command -v "$py" &>/dev/null; then
        ...
        if python3 -c "import sys; sys.exit(0 if sys.version_info >= (3,8) else 1)" 2>/dev/null; then
            ...
            success "Found Python ${PY_VER} at ${PYTHON_BIN}"
            break
        fi
    fi
done
...
# ─── 4. (Skipped) Create user ─────────────────────────────────────────────────
header "Step 4: System User"
info "Running as root — skipping dedicated user creation"
...
# ─── 6. Write the exporter script ─────────────────────────────────────────────
header "Step 6: Installing Exporter Script"

EXPORTER_PY="${INSTALL_DIR}/nvidia_exporter.py"

# Check if we have a local source to copy from
SCRIPT_SOURCE=""
for candidate in \
    "${LOCAL_SCRIPT}" \
    "$(dirname "$0")/nvidia_exporter.py" \
    "/home/$(logname 2>/dev/null || echo '')/nvidia-exporter/nvidia_exporter.py" \
    "/opt/nvidia-exporter-src/nvidia_exporter.py"; do
...
if [[ -n "$SCRIPT_SOURCE" ]]; then
    cp "${SCRIPT_SOURCE}" "${EXPORTER_PY}"
    success "Copied exporter script from ${SCRIPT_SOURCE}"
else
    info "Writing full-featured exporter script (embedded)..."
    cat > "${EXPORTER_PY}" << 'PYEOF'
#!/usr/bin/env python3
"""
...
SCRAPE_INTERVAL = int(os.environ.get("NVIDIA_SCRAPE_INTERVAL", "13"))
...
if __name__ == "__main__":
    main()
PYEOF
    success "Full-featured exporter script installed (39+ metrics, NVLink, MIG, datacenter support)"
fi

chmod 755 "${EXPORTER_PY}"

# ─── 7. Create virtualenv and install dependencies ────────────────────────────
header "Step 7: Setting Up Python Virtual Environment"

VENV_DIR="${INSTALL_DIR}/venv"

# Ensure python3-venv + ensurepip are available for the detected Python version
PY_VER_SHORT=$("${PYTHON_BIN}" -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")

# Always attempt to install venv packages upfront (ensurepip is part of python3.x-venv, not just venv)
if command -v apt-get &>/dev/null; then
    info "Ensuring python${PY_VER_SHORT}-venv is installed..."
    apt-get install -y "python${PY_VER_SHORT}-venv" "python${PY_VER_SHORT}-distutils" 2>/dev/null || \
    apt-get install -y python3-venv python3-distutils 2>/dev/null || \
    warn "apt-get install venv failed — will try fallback"
elif command -v yum &>/dev/null; then
    yum install -y "python${PY_VER_SHORT}-venv" 2>/dev/null || true
fi

# Try creating venv; fall back to virtualenv if ensurepip still unavailable
if "${PYTHON_BIN}" -m venv "${VENV_DIR}" 2>/dev/null; then
    info "venv created successfully"
else
    warn "python -m venv failed — installing virtualenv as fallback..."
    "${PYTHON_BIN}" -m pip install --quiet virtualenv 2>/dev/null || \
        pip3 install --quiet virtualenv 2>/dev/null || \
        error "Cannot install virtualenv. Run manually: apt install python${PY_VER_SHORT}-venv"
    "${PYTHON_BIN}" -m virtualenv "${VENV_DIR}" || \
        error "Failed to create virtual environment. Run: apt install python${PY_VER_SHORT}-venv"
fi

if [[ ! -f "${VENV_DIR}/bin/pip" ]]; then
    error "Failed to create virtual environment at ${VENV_DIR}. Try manually: apt install python${PY_VER_SHORT}-venv"
fi

"${VENV_DIR}/bin/pip" install --quiet --upgrade pip
"${VENV_DIR}/bin/pip" install --quiet prometheus-client
success "Virtual environment ready: ${VENV_DIR}"

# ─── 8. Set permissions ───────────────────────────────────────────────────────
header "Step 8: Setting Permissions"
chown -R root:root "${INSTALL_DIR}" "${LOG_DIR}"
chmod 755 "${INSTALL_DIR}"
chmod 644 "${EXPORTER_PY}"
# Allow reading /proc for process owner detection
chmod +r /proc/*/status 2>/dev/null || true
success "Permissions set"

# ─── 9. Create systemd service ────────────────────────────────────────────────
header "Step 9: Creating Systemd Service"

cat > "/etc/systemd/system/${SERVICE_NAME}.service" << EOF
[Unit]
Description=NVIDIA GPU Custom Prometheus Exporter
Documentation=https://github.com/nvidia-exporter
...
ExecStart=${VENV_DIR}/bin/python ${EXPORTER_PY}
Restart=always
RestartSec=10
...
# Environment
Environment=NVIDIA_EXPORTER_PORT=${EXPORTER_PORT}
Environment=NVIDIA_SCRAPE_INTERVAL=13
Environment=NVIDIA_EXPORTER_LOG=${LOG_DIR}/nvidia_exporter.log
...
success "Systemd service file created: /etc/systemd/system/${SERVICE_NAME}.service"
...
header "Step 11: Verifying Metrics Endpoint"
sleep 2
if command -v curl &>/dev/null; then
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:${EXPORTER_PORT}/metrics" 2>/dev/null || echo "000")
    if [[ "$HTTP_CODE" == "200" ]]; then
        METRIC_COUNT=$(curl -s "http://localhost:${EXPORTER_PORT}/metrics" 2>/dev/null | grep -c "^nvidia_" || echo 0)
        success "Metrics endpoint responding — ${METRIC_COUNT} nvidia_* metrics"
    else
        warn "Endpoint returned HTTP ${HTTP_CODE}. Service may still be initializing."
        warn "Test manually: curl http://localhost:${EXPORTER_PORT}/metrics | head -30"
    fi
else
    info "curl not found — test manually: curl http://localhost:${EXPORTER_PORT}/metrics"
fi

# ─── 12. Configure firewall (optional) ────────────────────────────────────────
header "Step 12: Firewall Configuration"
if command -v ufw &>/dev/null && ufw status 2>/dev/null | grep -q "Status: active"; then
    info "UFW detected. Adding rule for port ${EXPORTER_PORT}..."
    ufw allow "${EXPORTER_PORT}/tcp" comment "NVIDIA Exporter" 2>/dev/null && \
        success "UFW rule added for port ${EXPORTER_PORT}" || \
        warn "Could not add UFW rule — add manually: sudo ufw allow ${EXPORTER_PORT}/tcp"
elif command -v firewall-cmd &>/dev/null && firewall-cmd --state 2>/dev/null | grep -q "running"; then
    info "firewalld detected. Adding port ${EXPORTER_PORT}..."
    firewall-cmd --permanent --add-port="${EXPORTER_PORT}/tcp" 2>/dev/null && \
        firewall-cmd --reload 2>/dev/null && \
        success "firewalld rule added for port ${EXPORTER_PORT}" || \
        warn "Could not add firewalld rule — add manually"
else
    info "No active firewall detected (or ufw/firewalld not installed) — skipping"
fi
...
echo -e "${BOLD}${GREEN}║        NVIDIA Exporter Installed Successfully!       ║${NC}"
echo -e "${BOLD}${GREEN}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "  ${BOLD}Metrics URL:${NC}   http://${SERVER_IP}:${EXPORTER_PORT}/metrics"
echo -e "  ${BOLD}Service:${NC}       systemctl status ${SERVICE_NAME}"
echo -e "  ${BOLD}Logs:${NC}          journalctl -u ${SERVICE_NAME} -f"
echo -e "  ${BOLD}Log file:${NC}      ${LOG_DIR}/nvidia_exporter.log"
echo -e "  ${BOLD}Port:${NC}          ${EXPORTER_PORT} (DCGM-compatible)"
echo ""
echo -e "  ${BOLD}Add to Prometheus scrape config:${NC}"
echo -e "  ${CYAN}  - job_name: 'nvidia-exporter'"
echo -e "    static_configs:"
echo -e "      - targets: ['${SERVER_IP}:${EXPORTER_PORT}']${NC}"
echo ""
echo -e "  ${BOLD}Useful commands:${NC}"
echo -e "    sudo systemctl restart ${SERVICE_NAME}   # restart"
echo -e "    sudo systemctl stop    ${SERVICE_NAME}   # stop"
echo -e "    sudo systemctl disable ${SERVICE_NAME}   # disable autostart"
echo -e "    sudo $0 uninstall                        # uninstall"
echo ""

# ─── Restart nvidia-monitoring container if it was running before install ─────
if [[ "${NVIDIA_MONITORING_WAS_RUNNING}" == "true" ]]; then
    header "Restarting nvidia-monitoring Container"
    if command -v docker &>/dev/null; then
        docker start nvidia-monitoring 2>/dev/null && \
            success "Container 'nvidia-monitoring' restarted successfully" || \
            warn "Failed to restart container 'nvidia-monitoring' — start it manually: docker start nvidia-monitoring"
    fi
fi

# ─── Uninstall mode ───────────────────────────────────────────────────────────
if [[ "${1:-}" == "uninstall" ]]; then
    header "Uninstalling NVIDIA Exporter"
